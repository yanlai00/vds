...
class VFunctionRND:
    ...
    def __init__(self, inputs_tf, dimo, dimg, dimu, max_u, o_stats, g_stats, hidden, layers,
        ...
        with tf.variable_scope('V'):
            input_V = tf.concat(axis=1, values=[o, g])
            self.V_tf = nn(input_V, [self.hidden] * self.layers + [1], kernel_initializer=tf.initializers.glorot_uniform)
        with tf.variable_scope('V_target'):    
            input_V_2 = tf.concat(axis=1, values=[o, g])
            self.V_2_tf = nn(input_V_2, [self.hidden] * self.layers + [1], trainable=False, kernel_initializer=tf.initializers.glorot_normal)
...
class DoubleQFunctionDropout:
    ...
    def __init__(self, inputs_tf, dimo, dimg, dimu, max_u, o_stats, g_stats, hidden, layers,
        ...
        with tf.variable_scope('V'):
            input_V = tf.concat(axis=1, values=[o, g, self.u_tf / self.max_u])
            self.V_tf = nn(input_V, [self.hidden] * self.layers + [1], dropout=0.5)
